...
class HuffingtonpostSpider(scrapy.Spider):
    ...
    def generate_url(self, date, page_number=None):
        url = 'https://www.huffingtonpost.com/archive/' + date.strftime("%Y/%m/%d") + "/"
        if page_number:
        ...
        if response.status == 200:
            articles = response.xpath('//h2[@class="title"]/a/@href').extract()
            for url in articles:
    ...
    def parse_article(self, response):
        ...
        l.add_xpath('title', '//h2[@class="title"]/text()')
        l.add_xpath('text', '//div[(@class="body-text")]/p//text()')
